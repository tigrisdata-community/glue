...
// Generate creates a unique User-Agent string for outgoing HTTP requests.
func Generate(prefix, infoURL string) string {
	return fmt.Sprintf(
...
func (uat userAgentTransport) RoundTrip(r *http.Request) (*http.Response, error) {
	r.Header.Set("User-Agent", Generate(uat.prefix, uat.infoURL))
